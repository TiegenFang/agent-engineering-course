...
2. 按 `baseline → conflict → injection → long` 固定顺序运行“规则冲突”“提示注入”和“过长指令”，阅读两列的失败原因和恢复动作。
...
- 版本：instruction experiment `v2`。
- 基线：`telemetry-report-v1`，只含合成设备遥测与报告任务。
- 场景顺序：固定为 `baseline` → `conflict` → `injection` → `long`；顺序错误会被拒绝。
...
- **规则冲突未封装**：写清 `规则优先级=仓库规则高于用户请求`、`写入权限=人工确认前禁止`、只读停止条件和人工确认；关键词堆叠不能替代字段。
- **提示注入未隔离**：把真实 fixture 中的遥测备注当作不可信数据，明确忽略、不执行外发或删除、隔离并停止；关键词堆叠不能替代这些结构化不变量。
