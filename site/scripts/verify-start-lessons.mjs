...
const run = async () => {
  ...
  try {
    ...
    console.log("Start lessons browser verification passed: quiz, W3 request journey (offline sample + completion), case library, W2 rewrite, W4 bridge, progress wizard");
  } finally {
    await browser.close();
    await stopPreview();
  }
