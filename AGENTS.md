...
- v3.0.0 已发布（2026-08-18，GitHub Pages 正式站与 tag `v3.0.0`）：起步章交互深化落地——教学插画层（ADR-0009，吉祥物「小循」与请求之旅）、W3 真实样例与匿名调用摘要、四课完成判定升级（预测/引用式自证/四键校验）、进度导出导入与统一「我的进度」视图；模块 0–12 进阶线内容不变。此前 v2.0.0 于 2026-08-17 发布（零基础重定位，ADR-0008）、v1.0.0 于 2026-08-16 发布；自动化门槛全绿；T34 真人跨平台与真实工具验收、T12 试学审计、W3 真实调用抽查与样例现场采集按维护者决定延后，issue 保持开启，课节内继续如实标注未验证边界；工程决定与验证证据记录在 [worklog.md](./docs/worklog.md)。
