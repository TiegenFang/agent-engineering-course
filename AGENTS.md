...
- v3.0.0 已发布（2026-08-18，GitHub Pages 正式站与 tag `v3.0.0`）：起步章交互深化落地——教学插画层（ADR-0009，吉祥物「小循」与请求之旅）、W3 真实样例与匿名调用摘要、四课完成判定升级（预测/引用式自证/四键校验）、进度导出导入与统一「我的进度」视图；模块 0–12 进阶线内容不变。此前 v2.0.0 于 2026-08-17 发布（零基础重定位，ADR-0008）、v1.0.0 于 2026-08-16 发布；自动化门槛全绿；维护者已于 2026-08-18 完成 W3 真实调用现场抽查（匿名摘要与回放确认）与插画帧画面目视评审，样例数值的现场采集替换仍待执行；T34 真人跨平台与真实工具验收、T12 试学审计按维护者决定延后，issue 保持开启，课节内继续如实标注未验证边界；工程决定与验证证据记录在 [worklog.md](./docs/worklog.md)。
- 2026-08-17 维护者批准 [v3 总规格](./docs/specs/agent-engineering-course-v3.md)（起步章交互深化：证据先行、插画包壳），ticket V3-1…V3-8 经 GitHub Issues #38–#46 以多 agent 流水线实施完毕；实现按 ticket 推进、不顺手扩scope。
